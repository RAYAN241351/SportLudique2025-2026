...
**BIND (Berkeley Internet Name Domain)** est le serveur DNS le plus répandu dans le monde Linux/Unix.
La version actuelle, **BIND9**, gère :

- La résolution DNS (nom ↔ IP)
...
Elle permet de **distinguer les réponses internes et externes**, renforçant la sécurité et la flexibilité du réseau.
...
| /etc/bind/named.conf.options | Options globales (forwarders, récursivité…) |
...
**Déclaration des zone nécessaires**
...
```
## Créer le fichier de zone **db.interne** et **db.externe**
